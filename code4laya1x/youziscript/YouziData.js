...
var YouziDataModule;
(function (YouziDataModule) {
    /**
     * 底部banner类型
     */
    YouziDataModule.BANNER_TYPE = {
        MATRIX: 1,
        WX: 2,
        GAME: 3,
    ...
    /**
     * 已经请求过并进行了裁剪的动画对象json
     */
    YouziDataModule.AllDataAnimaTypeJson = {};
    /**
     * 交叉营销下行数据类型
     */
    var PAGE_TYPE = {
    ...
    YouziDataModule.YouziData = {
        SdkVersion: 'laya1.7.6-v6.1',
        resVersion: '1.00.00',
        debug: false,
        appid: '',
        channelId: 1002,
        ...
        _requestErrorCbs: [],
        _loadedCallBacks: [],
        _bannerType: YouziDataModule.BANNER_TYPE.MATRIX,
        ...
        _wxLaunch: function () {
            ...
            if (wxLaunchOptions.referrerInfo
                ...
                this.sendJumpToOpen(wxLaunchOptions.referrerInfo.extraData.orgAppid, wxLaunchOptions.referrerInfo.extraData.boxAppid, wxLaunchOptions.referrerInfo.extraData.locationIndex ? wxLaunchOptions.referrerInfo.extraData.locationIndex : YouziDataModule.BI_PAGE_TYPE.CUSTOM_COMPONENT);
            }
            else {
        ...
        _wxOnShow: function (wxOnShowRes) {
            ...
            if (wxOnShowRes.referrerInfo
                ...
                this.sendJumpToOpen(wxOnShowRes.referrerInfo.extraData.orgAppid, wxOnShowRes.referrerInfo.extraData.boxAppid, wxOnShowRes.referrerInfo.extraData.locationIndex ? wxOnShowRes.referrerInfo.extraData.locationIndex : YouziDataModule.BI_PAGE_TYPE.CUSTOM_COMPONENT);
            }
        },
        ...
        _loadData: function (cb) {
            ...
            var errCb = function () {
                ...
                for (var i = 0; i < self._requestErrorCbs.length; i++) {
                    var callback = self._requestErrorCbs[i];
                    if (callback)
                        callback();
                }
            };
            var cb2 = function (res) {
                    ...
                    for (var i = 0; i < res.info.recommendListBos.length; i++) {
                        ...
                        data.contentBos.forEach(function (item) {
                            if (!self.allBeRecommendGames.hasOwnProperty.call({}, item.appid)) {
                                self.allBeRecommendGames[item.appid] = Object.create(item);
                            }
                        });
                ...
                if (self._bannerType == YouziDataModule.BANNER_TYPE.SWITCH) {
        ...
        _request: function (methon, data, url, cb, errCb) {
        ...
        sendExposureLog: function (data, locationIndex) {
            ...
            var param = {
                ...
                "locationIndex": locationIndex ? locationIndex : YouziDataModule.BI_PAGE_TYPE.CUSTOM_COMPONENT,
        ...
        navigateToOtherGame: function (data, call) {
            ...
            var extraJson = {
                ...
                'locationIndex': data.locationIndex ? data.locationIndex : YouziDataModule.BI_PAGE_TYPE.CUSTOM_COMPONENT
            };
            //获取小程序路径
        ...
        notifyNavigateFailUIId: function () {
            switch (this.clickGameYouziUIId) {
                case YouziDataModule.YOUZI_UI_ID.Youzi_MainPush:
                case YouziDataModule.YOUZI_UI_ID.Youzi_BottomBanner:
                case YouziDataModule.YOUZI_UI_ID.Youzi_GuessLike:
                case YouziDataModule.YOUZI_UI_ID.Youzi_GuessLikeH:
                    YouziCenterModule.YouziCenter.getInstance().notifyNavigateToMini(this.clickGameYouziUIId);
                    break;
                default:
                    console.log('不需要取消');
                    break;
        ...
        sendGameToGame: function (_data) {
            ...
            var param = {
                ...
                "locationIndex": _data.locationIndex ? _data.locationIndex : YouziDataModule.BI_PAGE_TYPE.CUSTOM_COMPONENT,
        ...
        logNavigate: function (reqData, cb) {
            ...
            if (!this.debug) {
                this._request('POST', reqData, 'https://bi.log.lightlygame.com/recommend/', cb, null);
            }
        },
        ...
        _initBannerShow: function () {
            if (this._bannerType == YouziDataModule.BANNER_TYPE.MATRIX || this._bannerType == YouziDataModule.BANNER_TYPE.GAME || this._bannerType == YouziDataModule.BANNER_TYPE.WX) {
            ...
            else if (this._bannerType == YouziDataModule.BANNER_TYPE.SWITCH) {
                for (var i = 0; i < this._bannerSwitchs.length; i++) {
                    var banner = this._bannerSwitchs[i];
                    if (banner && banner.bannerType == YouziDataModule.BANNER_TYPE.MATRIX) {
                        banner.showBanner();
                    }
                    else if (banner) {
                        banner.hideBanner();
        ...
        refreshBannerSwitch: function () {
            if (!this.curBannerType) {
                this.curBannerType = YouziDataModule.BANNER_TYPE.WX;
            }
            this.curBannerType = this.curBannerType == YouziDataModule.BANNER_TYPE.WX ? YouziDataModule.BANNER_TYPE.MATRIX : YouziDataModule.BANNER_TYPE.WX;
            for (var i = 0; i < this._bannerSwitchs.length; i++) {
                var banner = this._bannerSwitchs[i];
        ...
        _destroyUnuseWxBanner: function (b) {
            if (b.bannerType == YouziDataModule.BANNER_TYPE.WX) {
                for (var i = 0; i < this._bannerSwitchs.length; i++) {
                    var banner = this._bannerSwitchs[i];
                    if (banner && banner.bannerType == YouziDataModule.BANNER_TYPE.WX) {
                        banner.destroySelf();
...
})(YouziDataModule || (YouziDataModule = {}));
/**
 * 对象合并polyfill
 * */
